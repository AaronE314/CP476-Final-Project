...
export class ItemDisplayBox extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            discount: 0.1,
            colours: [
                {name:"Default", hex:"#F0F0F0", link:"#F0F0F0"},
                {name:"Black", hex:"#000000", link: "#000000"},
                {name:"Yellow", hex:"#FFF500", link: "#FFF500"}
            ],
    ...
    render(){
        ...
        return <div className={styles.DisplayItem}>
            ...
            <img src={this.state.colours[0].link} id="displayImage"></img>
            ...
            <div className={styles.DisplayInfo}>
                ...
                {(this.state.discount > 0) ? <p className={styles.DisplayDiscount} style={{background: this.state.highlightColor}}>${this.getAdjustedPrice()}</p> : null}
                ...
                <div className={styles.DisplayColorContainer}>
                    {this.state.colours.map((item, i) => {
                        return <div className={styles.DisplayColorOption} style={{background: item.hex}} onMouseOver={() => {this.hoverOn(item.link)}} onMouseOut={() => {this.hoverOff()}}></div>
                    })}
                </div>
            </div>
        </div>
